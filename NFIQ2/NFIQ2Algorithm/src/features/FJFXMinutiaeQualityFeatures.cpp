...
FJFXMinutiaeQualityFeature::computeFeatureData(
  ...
  try {
    ...
    // compute minutiae quality based on Mu feature computated at minutiae
    // positions
    ...
    res_mu.featureData.featureDataDouble =
        (double)vecRanges.at(2) / (double)minCnt;
    featureDataList.push_back(res_mu);

    // compute minutiae quality based on OCL feature computed at minutiae
    // positions
    std::vector<MinutiaData> vecOCLMinQualityData =
        computeOCLMinQuality(fmds, minCnt, BS_OCL, fingerprintImage);
    ...
    res_ocl.returnCode = 0;
    // return relative value in relation to minutiae count
    res_ocl.featureData.featureDataDouble =
        (double)vecRangesOCL.at(4) / (double)minCnt;
    featureDataList.push_back(res_ocl);
...
FJFXMinutiaeQualityFeature::computeOCLMinQuality(
  ...
  for (unsigned int i = 0; i < minCount; i++) {
    ...
    // if in edge reason -> don't center around minutiae but take full block
    // that is closest
    if ((leftX + bs) > (int)fingerprintImage.m_ImageWidth) {
