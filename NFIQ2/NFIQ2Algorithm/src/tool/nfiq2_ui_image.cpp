...
NFIQ2UI::getImages(const std::string& path,
    ...
    try {
        ...
        logger->printError(path, 0, 255, error.append(e.what()) + "'", false,
                           false);
    }
...
NFIQ2UI::getImagesFromAN2K(const BE::Memory::uint8Array& dataArray,
    ...
    for (const auto& cap : captures) {
        ...
        if (fingerPosition >= 0 && fingerPosition <= 12) {
            ...
            if (imageDPI != an2kDPI) {
                ...
                try {
                    ...
                    img = std::make_shared<BE::Image::Raw>(
                        img->getRawData(), img->getDimensions(),
                        img->getColorDepth(), img->getBitDepth(),
                        cap.getImageResolution(), img->hasAlphaChannel(),
            ...
            logger->debugMsg("Invalid fingerprint position provided: " + name +
                             "_" + std::to_string(fingerPosition));
...
NFIQ2UI::getImagesFromANSI2004(const BE::Memory::uint8Array& dataArray,
    ...
    try {
        ...
        ansi2004 = std::make_shared<BE::DataInterchange::ANSI2004Record>(
            empty, dataArray);
    } catch (const BE::Error::Exception& e) {
    ...
    for (uint64_t i = 0; i < printCount; i++) {
        ...
        logger->debugMsg("Successfully parsed image from ANSI2004 Record: " +
                         name + "_" + std::to_string(fingerPosition));
