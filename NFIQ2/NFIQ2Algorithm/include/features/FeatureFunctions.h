...
void
ridgesegment(const cv::Mat& Image, int blksze, double thresh,
...
double
ocl(double a, double b, double c);

uint8_t
allfun(const cv::Mat& image);

void
getRotatedBlock(const cv::Mat& block, const double orientation, bool padFlag,
                cv::Mat& rotatedBlock);

void
getRidgeValleyStructure(const cv::Mat& blockCropped,
                        std::vector<uint8_t>& ridval, std::vector<double>& dt);
void
Conv2D(const cv::Mat& im, const cv::Mat& filter, cv::Mat& ConvOut,
