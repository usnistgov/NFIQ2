...
class ImgProcROIFeature : BaseFeature {
 public:
  struct ImgProcROIResults {
    ...
    std::vector<cv::Rect>
        vecROIBlocks;  ///< the detected ROI blocks with position and size
    unsigned int noOfROIPixels;    ///< the number of ROI pixels detected in the
                                   ///< image (not blocks)
    ...
    double stdDevOfROIPixels;  ///< the standard deviation of all grayvalues of
                               ///< all ROI pixels
  };
