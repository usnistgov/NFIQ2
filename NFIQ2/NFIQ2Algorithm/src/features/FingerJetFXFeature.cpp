    ...
    bool &templateCouldBeExtracted)
{
	...
	if (FRFXLLGetMinutiaInfo(hFeatureSet, &num_minutia, &resolution_ppi) !=
	    FRFXLL_OK) {
		// return features
		fd_min_cnt_comrect200x200.featureDataDouble =
		    0; // no minutiae found
	...
	mdata = (FRFXLL_Basic_19794_2_Minutia *)calloc(
	    num_minutia, sizeof(FRFXLL_Basic_19794_2_Minutia));
	...
	FingerJetFXFeature::FJFXROIResults roiResults =
	    computeROI( // Will not be mdata - needs to be a vector of minutiae
    ...
    std::vector<FingerJetFXFeature::Object> vecRectDimensions)
{
	...
	for (unsigned int i = 0; i < vecRectDimensions.size(); i++) {
		...
		for (unsigned int k = 0; k < minCount; k++) {
			if (vecMData[k].x >= startX && vecMData[k].x <= endX &&
			    vecMData[k].y >= startY && vecMData[k].y <= endY) {
