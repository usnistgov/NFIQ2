...
OFFeature::computeFeatureData(
  ...
  try {
    ...
    // loqall = blockproc(blkorient, [1 1], @orientangdiff, 'BorderSize',
    // [border border], 'TrimBorder', false); add a border of zeros around the
    // blkorient array, since the matlab blockproc function pads with zeros at
    // the edges.
    Mat paddedBlkorient;
    copyMakeBorder(blkorient, paddedBlkorient, 1, 1, 1, 1, BORDER_CONSTANT, 0);

    // for each point in the original blkorient array, compute the orientation
    // angle difference with its immediate neighbors all around.

    Mat loqall(blkorient.rows, blkorient.cols, CV_64F);
    const double bsize =
        9;  // The center point plus its immediate neighbors forms a 3x3 block
    ...
    for (int i = 1; i <= blkorient.rows; i++) {
      for (int j = 1; j <= blkorient.cols; j++) {
        ...
        // Subtract the input block from the center value and get its absolute
        // value.
        Mat blockAbsDiff;
    ...
    // % overlapping window: if one of the surrouding blocks from which the
    // anglediff was computed % is in background, exclude whole window from
    // comp. maskBloqseg = logical(blkproc(maskBseg, [1 1], [border border],
    // allfun));
    Mat paddedMaskBseg;
    ...
    // % global orientation flow quality score (GOQS) only from FOREGROUND (all
    // neighbouring) % blocks (background = 0) goqs = mean(loqs(maskBloqseg));
    ...
    addHistogramFeatures(featureDataList, "OF_Bin10_", histogramBins10,
                         dataVector, 10);
#endif
