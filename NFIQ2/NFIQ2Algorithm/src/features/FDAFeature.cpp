...
FDAFeature::computeFeatureData(
  ...
  Mat img = Mat(fingerprintImage.m_ImageHeight, fingerprintImage.m_ImageWidth,
  ...
  try {
    ...
    // Image processed NOT from beg to end but with a border around - can't be
    // vectorized:(
    int br = 0;
    int bc = 0;
    for (int r = blkoffset; r < rows - (blksize + blkoffset - 1);
         r += blksize) {
      for (int c = blkoffset; c < cols - (blksize + blkoffset - 1);
           c += blksize) {
        ...
        if (mask == 1) {
          ...
          blkwim =
              img(Range(r - blkoffset, min(r + blksize + blkoffset, img.rows)),
                  Range(c - blkoffset, min(c + blksize + blkoffset, img.cols)));
          fdas.at<double>(br, bc) = fda(blkwim, blkorient.at<double>(br, bc),
    ...
    //
    // Each maskBseg entry is true if all elements of the corresponding maskB1
    // are non-zero, false if one or more is zero.  Above matlab code
    // substitutes a NaN for fdas entries that correspond to masks where one or
    // more entries were 0.  It then takes the mean of all entries that are not
    // NaNs.
    //
    // In the code below, each maskBseg entry is 1 if all entries are non-zero,
    // and 0 otherwise.
    //  MaskBseg is passed to the mask parameter of the OpenCV mean function, so
    //  that only fdas
    ...
    addHistogramFeatures(featureDataList, "FDA_Bin10_", histogramBins10,
                         dataVector, 10);
#endif
...
fda(const Mat& block, const double orientation, const int v1sz_x,
  ...
  if (icBlock != cBlock) {
    ...
    std::cerr
        << "warning: Wrong block size! Consider block with size of even number"
        << std::endl;
  }
  ...
  // extract slanted block by cropping the rotated image: To ensure that rotated
  // image does not contain any invalid regions.
  //     Matlab:  blockCropped =
  //     blockRotated(cBlock-(xoff-1):cBlock+xoff,cBlock-(yoff-1):cBlock+yoff);
  //     % v2
  // Note: Matlab uses matrix indices starting at 1, OpenCV starts at 0.  Also,
  // OpenCV ranges are open-ended on the upper end.

  Mat blockCropped =
      blockRotated(Range((icBlock - (xoff - 1) - 1), (icBlock + xoff)),
  ...
  for (int r = 0; r < blockCropped.rows; r++) {
    ...
    Mat roi = Mat(blockCropped,
                  Rect(0, r, blockCropped.cols, 1));  // x,y, width, height
    Scalar s = mean(roi);
  ...
  // create output
  copyMakeBorder(t.t(), tmpM, 0, m - t.cols, 0, n - t.rows, BORDER_CONSTANT,
                 Scalar::all(0));
  ...
  return (mVal + 0.3 * (amp.at<double>(0, mLoc.x - 1) +
