...
RandomForestML::initModule(const std::string& params) {
  ...
  FileStorage fs(params.c_str(), FileStorage::READ | FileStorage::MEMORY |
                                     FileStorage::FORMAT_YAML);
  // now import data structures
#if CV_MAJOR_VERSION <= 2
  m_pTrainedRF = new CvRTrees();
  m_pTrainedRF->read(fs.fs,
                     cvGetFileNodeByName(fs.fs, NULL, "my_random_trees"));
#else
...
std::string
RandomForestML::initModule(const std::string& fileName,
  ...
  try {
    ...
    if (fileHash.compare(hash) != 0) {
      ...
      throw NFIQ::NFIQException(
          ...
          "The trained network could not be initialized! Error: " + hash);
    }
    return hash;
  } catch (const cv::Exception& e) {
    std::cout << e.msg.c_str() << std::endl;
...
RandomForestML::evaluate(
  ...
  try {
    ...
    for (it_feat = featureVector.begin(); it_feat != featureVector.end();
         it_feat++) {
...
#if CV_MAJOR_VERSION <= 2
    // returns probability that between 0 and 1 that result belongs to second
    // class
    ...
    // returns probability that between 0 and 1 that result belongs to second
    // class
    float prob = m_pTrainedRF->predict(sample_data, noArray(),
