...
OFFeature::computeFeatureData(
    ...
    try {
        ...
        Mat paddedBlkorient;
        copyMakeBorder(blkorient, paddedBlkorient, 1, 1, 1, 1, BORDER_CONSTANT,
                       0);
        ...
        for (int i = 1; i <= blkorient.rows; i++) {
            for (int j = 1; j <= blkorient.cols; j++) {
                ...
                // upper end
                Mat blkROI =
                    paddedBlkorient(Range(i - 1, i + 2), Range(j - 1, j + 2));
        ...
        for (int i = 0; i < loqall.rows; i++) {
            for (int j = 0; j < loqall.cols; j++) {
                if (maskBloq.at<uint8_t>(i, j) == 1) {
                    loqs.at<double>(i, j) =
                        (loqall.at<double>(i, j) - angmin) / angdiff;
