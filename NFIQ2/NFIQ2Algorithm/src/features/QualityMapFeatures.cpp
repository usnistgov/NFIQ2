...
QualityMapFeatures::computeFeatureData(
    ...
    try {
        ...
        Mat orientationMapImgFilter =
            computeOrientationMap(img, true, coherenceSumFilter,
                                  coherenceRelFilter, 16, imgProcResults);
    ...
    for (int i = 0; i < img.rows; i += bs) {
        for (int j = 0; j < img.cols; j += bs) {
            ...
            if (bFilterByROI) {
                ...
                for (unsigned int k = 0; k < roiResults.vecROIBlocks.size();
                     k++) {
            ...
            for (int k = i; k < (i + actualBS_Y); k++) {
                for (int l = j; l < (j + actualBS_X); l++) {
                    omImg.at<uchar>(k, l) = angleDegree;  // set to angle
                                                          // value
                }
            }
        }
