...
fda(const Mat& block, const double orientation, const int v1sz_x,
    ...
    for (int r = 0; r < blockCropped.rows; r++) {
        ...
        Mat roi = Mat(blockCropped, Rect(0, r, blockCropped.cols,
                                         1));  // x,y, width, height
        Scalar s = mean(roi);
