...
NFIQ2UI::executeSingle(std::shared_ptr<BE::Image::Image> img,
    ...
    if (flags.force) {
      ...
      if (interactive && !flags.force) {
        ...
        const std::string prompt =
            "This Image is not 8 bit grayscale. Would "
  ...
  if ((resolution.xRes != resolution.yRes) || (imageDPI != 500)) {
    ...
    if (flags.force) {
      ...
      if (interactive && !flags.force) {
        const std::string prompt =
            "This Image is not 500 PPI. Would you like "
  ...
  if (corereturn.qualityScore > 100) {
    logger->printError(name, fingerPosition, corereturn.qualityScore,
...
NFIQ2UI::coreCompute(const NFIQ::FingerprintImageData& wrappedImage,
  ...
  const CoreReturn corereturn{featureVector, featureTimings, actionableQuality,
                              qualityScore};
  return corereturn;
}
        ...
        if (flags.recursion &&
          ...
          if (NFIQ2UI::isRecordStore(NFIQ2UI::removeSlash(dirname) + "/" +
            ...
            NFIQ2UI::executeRecordStore(
                NFIQ2UI::removeSlash(dirname) + "/" + en->d_name, flags, model,
                logger);
          } else {
    ...
    for (unsigned int i{0}; i < upperThreadBound; ++i) {
      try {
        threads.emplace_back(
            std::bind(&recordStoreConsume, filename, std::ref(splitQueue),
...
main(int argc, char** argv) {
  ...
  const NFIQ::NFIQ2Algorithm model("nfiq2rf.yaml",
