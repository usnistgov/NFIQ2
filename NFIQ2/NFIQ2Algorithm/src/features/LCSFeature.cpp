  ...
  try {
    ...
    for (int r = blkoffset; r < rows - (blocksize + blkoffset - 1);
      ...
      for (int c = blkoffset; c < cols - (blocksize + blkoffset - 1);
        ...
        blkwim =
            img(Range(r - blkoffset, min(r + blocksize + blkoffset, img.rows)),
                Range(c - blkoffset, min(c + blocksize + blkoffset, img.cols)));
        lcs.at<double>(br, bc) = loclar(blkwim, blkorient.at<double>(br, bc),
                                        v1sz_x, v1sz_y, scannerRes, padFlag);
...
loclar(Mat& block, const double orientation, const int v1sz_x, const int v1sz_y,
  ...
  if (!changeIndex.empty()) {
    ...
    // ridge-valley thickness: NOTE: first and last region can be INCOMPLETE
    // -should be somehow excluded from the test 1: all should fall in (except
    // first/last) 2: majority 3: mean/median of all if all(NWr >= NWrmin) &&
    // all(NWr <= NWrmax) && all(NWv >= NWvmin) && all(NWv <= NWvmax)
    ...
    if ((muNWr.val[0] >= NWrmin) && (muNWr.val[0] <= NWrmax) &&
      ...
      // ridge region
      //      ridmat = v2(:,ridval==1); % matrix of ridge pxs (according to v3
      //      and dt1)
      // remove columns in v2 that do not correspond to ridges

      // Compute the number of pixels in ridge regions that exceed their
      // threshold Likewise, compute the number of pixels in valley regions that
      ...
      for (int i = 0; i < v2.cols; i++) {
        if (ridval[i] == 1)  // ridges
        {
          ...
          ridgeGood += countNonZero(
              ridgecol);  // no. pixels in column that are above the threshold
      ...
      double beta =
          static_cast<double>(ridgeGood) / static_cast<double>(ridgePixelCount);
