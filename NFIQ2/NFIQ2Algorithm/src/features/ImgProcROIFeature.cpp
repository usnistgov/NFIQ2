...
ImgProcROIFeature::computeROI(cv::Mat& img, unsigned int bs) {
...
#if CV_MAJOR_VERSION <= 2
      drawContours(filledImg, contours, idx, Scalar(0, 0, 0, 0), CV_FILLED, 8,
                   hierarchy);
#else
      drawContours(filledImg, contours, idx, Scalar(0, 0, 0, 0), cv::FILLED, 8,
                   hierarchy);
#endif /* CV_MAJOR_VERSION */
    }
#if CV_MAJOR_VERSION <= 2
  ...
  for (unsigned int i = 0; i < vecRects.size(); i++) {
    if (i != maxIdx) {
      ...
      floodFill(threshImg2, Point(vecPoints.at(i).x, vecPoints.at(i).y),
  ...
  for (int i = 0; i < threshImg2.rows; i++) {
    for (int j = 0; j < threshImg2.cols; j++) {
      if (((int)threshImg2.at<uchar>(i, j)) == 0) {
        ...
        sumSquare +=
            (((double)x - meanOfROIPixels) * ((double)x - meanOfROIPixels));
      }
    }
  }
  ...
  for (unsigned int i = 0; i < height; i += bs) {
    for (unsigned int j = 0; j < width; j += bs) {
      ...
      if (m.val[0] < 255) {
        // take block
#if CV_MAJOR_VERSION <= 2
        rectangle(bsImg, Point(j, i), Point(j + takenBS_X, i + takenBS_Y),
