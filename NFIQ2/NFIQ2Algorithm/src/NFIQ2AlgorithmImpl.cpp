...
NFIQ2Algorithm::Impl::Impl(const std::string& fileName,
...
NFIQ2Algorithm::Impl::computeQualityFeatures(
  ...
  double sigma = 255.0;
  std::list<NFIQ::QualityFeatureResult> muFeatures =
      muFeatureModule.computeFeatureData(rawImage, sigma);

  // find Mu feature to get its value and return actionable feedback for empty
  // images
  ...
  if (bComputeActionableQuality) {
    ...
    if (isEmptyImage || isUniformImage) {
      ...
      // return empty feature vector since this actionable feedback results in a
      // quality score of 255 other feature values will not be computed in that
      // case
  ...
  std::list<NFIQ::QualityFeatureResult> fdaFeatures =
  ...
  if (bComputeActionableQuality) {
    for (it_fjfxFeatures = fjfxFeatures.begin();
      ...
      if (it_fjfxFeatures->featureData.featureID.compare(
        ...
        NFIQ::ActionableQualityFeedback fb;
        fb.actionableQualityValue =
            it_fjfxFeatures->featureData.featureDataDouble;
        fb.identifier = NFIQ::
  ...
  std::list<NFIQ::QualityFeatureResult> fjfxMinQualFeatures =
      fjfxMinQualFeatureModule.computeFeatureData(rawImage, fjfxTemplateData,
                                                  fjfxTemplateSize,
                                                  templateCouldBeExtracted);
  ...
  std::list<NFIQ::QualityFeatureResult> roiFeatures =
  ...
  fb_roi.actionableQualityValue =
      imgProcResults
          .noOfROIPixels;  // absolute number of ROI pixels (foreground)
  fb_roi.identifier =
      NFIQ::ActionableQualityFeedbackIdentifier_SufficientFingerprintForeground;
  ...
  std::list<NFIQ::QualityFeatureResult> lcsFeatures =
  ...
  std::list<NFIQ::QualityFeatureResult> oclFeatures =
  ...
  std::list<NFIQ::QualityFeatureResult> ofFeatures =
      ofFeatureModule.computeFeatureData(rawImage);

  // append to feature vector
  std::list<NFIQ::QualityFeatureResult>::iterator it_ofFeatures;
  for (it_ofFeatures = ofFeatures.begin(); it_ofFeatures != ofFeatures.end();
       ++it_ofFeatures) {
  ...
  std::list<NFIQ::QualityFeatureResult> rvupFeatures =
...
NFIQ2Algorithm::Impl::computeQualityScore(
  ...
  try {
    ...
    std::list<NFIQ::QualityFeatureData> featureVector = computeQualityFeatures(
        croppedRawImage, bComputeActionableQuality, actionableQuality,
...
NFIQ2Algorithm::Impl::getAllActionableIdentifiers() {
...
std::vector<std::string>
NFIQ2Algorithm::Impl::getAllQualityFeatureIDs() {
...
std::vector<std::string>
NFIQ2Algorithm::Impl::getAllSpeedFeatureGroups() {
