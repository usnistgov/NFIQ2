...

{
  ...
  ***/
  ...
  // OpenCV: Result of comparison is an 8-bit single channel mask whose elements
  // are
  ...
  if (_maskIndex.needed()) {
    /***Create the mask vector indicating ridge-like regions: get the linear
    indices of non-zero elements of the matrix Matlab:    maskind = find(mask);
    ***/
...
covcoef(const cv::Mat& imblock, double& a, double& b, double& c,
  ...
  ***/
  ...
  if (compMethod == CENTERED_DIFFERENCES) {
    ...
    /* estimate the gradient in the x direction (across the columns) by
...
//////////////////////////////////////////////////////////////
void
getRotatedBlock(const Mat& block, const double orientation, bool padFlag,
                Mat& rotatedBlock) {
  ...
  try {
    ...
    //   Matlab:  blockRotated = imrotate(block, rad2deg(orientation),
    //   'nearest', 'crop');
...
getRidgeValleyStructure(const Mat& blockCropped, std::vector<uint8_t>& ridval,
  ...
  // average profile of blockCropped: Compute average of each column to get a
  // projection of the grey values down the ridges.
  //    Matlab:  v3 = mean(blockCropped);
  //    Note: If A is a matrix, mean(A) treats the columns of A as vectors,
  //    returning
  ...
  // coefficients
  //   x = 1:length(v3);
  // % Append a column of ones before dividing to include an intercept, dt1 =
  ...
  try {
    ...
    std::stringstream ssErr;
    ssErr << "Exception during ridge/valley processing: solve(DECOMP_QR) "
          << e.what();
    throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError,
                              ssErr.str());
  }
  ...
  for (int i = 0; i < dt1.rows; i++) {
    for (int j = 0; j < dt1.cols; j++) {
      dt1.at<double>(i, j) =
          round(dt1.at<double>(i, j) * 100000000000) / 100000000000;
    }
  }
#else
...
Conv2D(const Mat& imDFT, const Mat& filter, Mat& ConvOut, const Size& imageSize,
  ...
  // Input is the DFT of an image, and a complex-valued filter.  We compute the
  // DFT of the filter, multiply the two DFTs, then compute the inverse
  // transform.
...
computeNumericalGradients(const cv::Mat& mat, cv::Mat& grad_x,
  ...
  // to get the y-gradient: take the x-gradient of the transpose matrix and
  // transpose it again
...
addHistogramFeatures(std::list<NFIQ::QualityFeatureResult>& featureDataList,
  ...
  if (myBinCount != binCount) {
    std::stringstream s;
    s << "Wrong histogram bin count for " << featurePrefix << ". Should be "
      << binCount << " but is " << myBinCount;
...
void
addSamplingFeatureNames(std::list<std::string>& featureNames,
                        const char* prefix) {
  for (int i = 0; i < maxSampleCount; i++) {
    std::stringstream s;
    s << prefix << i;
...
void
addHistogramFeatureNames(std::list<std::string>& featureNames,
