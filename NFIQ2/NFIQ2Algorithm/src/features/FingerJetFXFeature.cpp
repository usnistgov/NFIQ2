...
FingerJetFXFeature::computeFeatureData(
  ...
  if (createContext(&hCtx) != FRFXLL_OK) {
    throw NFIQ::NFIQException(
        NFIQ::e_Error_FeatureCalculationError_FJFX_CannotCreateContext,
        "Cannot create context of feature extraction (create context failed).");
  }
  if (hCtx == NULL) {
    throw NFIQ::NFIQException(
        NFIQ::e_Error_FeatureCalculationError_FJFX_CannotCreateContext,
        "Cannot create context of feature extraction (hCtx is NULL).");
  }

  // extract feature set
  FRFXLL_RESULT fxRes = FRFXLLCreateFeatureSetInPlaceFromRaw(
      hCtx, (unsigned char*)localFingerprintImage.data(),
      localFingerprintImage.size(), localFingerprintImage.m_ImageWidth,
      localFingerprintImage.m_ImageHeight, localFingerprintImage.m_ImageDPI,
  ...
  FRFXLL_OUTPUT_PARAM_ISO_ANSI param = {
      sizeof(FRFXLL_OUTPUT_PARAM_ISO_ANSI),
      CBEFF,
      localFingerprintImage.m_FingerCode,
      0,
      dpcm,
      dpcm,
      (unsigned short)localFingerprintImage.m_ImageWidth,
      (unsigned short)localFingerprintImage.m_ImageHeight,
      0,
      0,
      0 /*live-scan plain*/
  };
  if (FRFXLLExport(hFeatureSet, FRFXLL_DT_ISO_FEATURE_SET, &param, templateData,
                   &templateSize) != FRFXLL_OK) {
    // return features
    fd_min_cnt_comrect200x200.featureDataDouble = 0;  // no minutiae found
    res_min_cnt_comrect200x200.featureData = fd_min_cnt_comrect200x200;
    res_min_cnt_comrect200x200.returnCode = 0;
  ...
  if (templateSize < 28) {
    throw NFIQ::NFIQException(
        NFIQ::e_Error_FeatureCalculationError_FJFX_ISOTemplateTooSmall,
  ...
  if (new_fmr(FMR_STD_ISO, &fmr) < 0) {
    throw NFIQ::NFIQException(
        NFIQ::e_Error_FeatureCalculationError_FJFX_CannotAllocateFMR,
  ...
  if (rcount == 0) {
    ...
    throw NFIQ::NFIQException(
        NFIQ::e_Error_FeatureCalculationError_FJFX_NoMinutiaeRecords,
  ...
  if (get_fvmrs(fmr, fvmrs) != rcount) {
    ...
    throw NFIQ::NFIQException(
        NFIQ::e_Error_FeatureCalculationError_FJFX_CannotGetMinutiaeRecords,
  ...
  if (get_fmds(fvmrs[0], fmds) != minCnt) {
    ...
    throw NFIQ::NFIQException(
        NFIQ::e_Error_FeatureCalculationError_FJFX_CannotGetMinutiaeData,
  ...
  FingerJetFXFeature::FJFXROIResults roiResults =
      computeROI(fmds, minCnt, 32, fingerprintImage, vecRectDimensions);
  double noOfMinInRect200x200 = 0;
  for (unsigned int i = 0; i < roiResults.vecNoOfMinutiaeInRectangular.size();
       i++) {
    if (roiResults.vecNoOfMinutiaeInRectangular.at(i).comType ==
        e_COMType_MinutiaeLocation) {
      if (roiResults.vecNoOfMinutiaeInRectangular.at(i).width == 200 &&
          roiResults.vecNoOfMinutiaeInRectangular.at(i).height == 200) {
        noOfMinInRect200x200 =
            roiResults.vecNoOfMinutiaeInRectangular.at(i).noOfMinutiae;
      }
    }
  }
...
FingerJetFXFeature::computeROI(
  ...
  // get number of minutiae that lie inside a block defined by the Centre of
  // Mass (COM)
  ...
  for (unsigned int i = 0; i < vecRectDimensions.size(); i++) {
    ...
    unsigned int comRectWidthHalf =
        vecRectDimensions.at(i).width / 2;  // round down
    // check boundaries
    ...
    for (unsigned int k = 0; k < minCount; k++) {
      if (fmds[k]->x_coord >= startX && fmds[k]->x_coord <= endX &&
          fmds[k]->y_coord >= startY && fmds[k]->y_coord <= endY) {
