...
namespace NFIQ
{
	...
	*/
	typedef enum feature_data_type_e
	{
		...
		e_QualityFeatureDataTypeDouble = 1,		///< Feature has floating point format
	...
	*/
	typedef struct feature_data_t
	{
		...
		NFIQ::QualityFeatureDataType	featureDataType;	///< The type of feature data
		double							featureDataDouble;	///< The feature value in floating point format (if type is e_QualityFeatureDataTypeDouble)
		NFIQ::Data						featureDataBinary;	///< The feature value in binary data format (if type is e_QualityFeatureDataTypeBinary)
	} QualityFeatureData;
	...
	*/
	typedef struct feature_speed_t
	{
		...
		std::list<std::string>			featureIDs;			///< The unique IDs of the features that are used for determing the speed
		double							featureSpeed;		///< The speed of feature data computation in milliseconds
	} QualityFeatureSpeed;
	...
	*/
	typedef struct database_information_t
	{
		...
		uint32_t		databaseDimension;	///< number of fingerprints which are stored in the original database
	} DatabaseInformation;
	...
	*/
	typedef struct quality_feature_result_t
	{
		...
		uint32_t returnCode;					///< The return code of the quality feature extraction operation
	...
	*/
	typedef struct utility_result_t
	{
		...
		uint32_t returnCode;				///< The return code of the utility computation operation
	} UtilityResult;
	...
	*/
	...
	{
		std::string databaseID;				///< The database ID where the fingerprint image is stored
		NFIQ::ImageID fingerprintImageID;	///< The ID of the fingerprint image
		NFIQ::UtilityResult utilityResult;	///< The result of the utility computation (value + return code)
	} UtilitySample;

	/**
	* This type represents the combination of a provider and a list of utility samples
	*/
	typedef struct utility_provider_content_t
	{
		std::string providerID;							///< The provider ID for which the utility was computed
		std::list<NFIQ::UtilitySample> utilitySamples;	///< A list of utility values and its information (imageID, return code)
	...
	*/
	typedef struct utility_content_t
	{
		...
		std::list<std::string> providerIDs;			///< A list of provider IDs
		std::list<NFIQ::UtilitySample> samples;		///< Results of the utility computation for images of the database
	} UtilityContent;
	...
	*/
	...
	{
		NFIQ::ImageID fingerprintImageID;			///< The ID of the fingerprint image
		NFIQ::QualityFeatureResult featureResult;	///< The result of the quality feature computation (value + return code)
	...
	*/
	typedef struct quality_feature_content_t
	{
		...
		std::list<NFIQ::QualityFeatureSample> samples;	///< Results of the quality feature computation for images of the database
	...
	*/
	...
	{
		NFIQ::ImageID probeImageID;					///< The ID of the probe/verification fingerprint image
		std::string comparisonType;					///< The comparison type: "g" ... Genuine comparison, "i" ... Impostor comparison, "x" ... Cross-comparison between same person
		NFIQ::ComparisonResult comparisonResult;	///< Achieved score and return code between probe and reference image
	...
	*/
	...
	{
		NFIQ::ImageID referenceImageID;					///< The ID of the reference/enrolment fingerprint image
		std::list<NFIQ::ComparisonProbeResult> probes;	///< A list of probe images + results for which comparisons where conducted
	...
	*/
	...
	{
		std::string databaseID;								///< The ID of the database
		std::string providerID;								///< The ID of the provider used for comparison scores computation
		std::list<NFIQ::ComparisonScoresSample> references;	///< Results of the comparison scores computation for this reference fingerprint image
	...
	*/
	...
	{
		std::list<NFIQ::QualityFeatureData> featureDataVector;	///< The quality feature data vector used for training
	...
	*/
	typedef struct evaluation_result_t
	{
		...
		double deviation;					///< The deviation of the quality score and utility value
	...
	*/
	...
	/**
	* This type represents a list of weight image IDs for a dedicated database = training ID
	*/
	typedef struct training_id_t
	{
		std::string databaseID;						///< The ID of the database
		std::list<NFIQ::WeightImageID> imageIDs; 	///< A list of image IDs + their weights
	} TrainingID;
	...
	*/
	...
	{
		NFIQ::ImageID fingerprintImageID; 	///< The ID of the fingerprint image
		std::string databaseID;				///< The ID of the database where image is stored
	} DatabaseImageID;

	/**
	* This type represents a list of image IDs and an assigned database ID for evaluation
	*/
	typedef struct evaluation_id
	{
		std::string databaseID;				///< The ID of the database where image is stored
		std::list<NFIQ::ImageID> imageIDs; 	///< A list of image IDs
	} EvaluationID;
}
