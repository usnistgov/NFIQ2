...
RVUPHistogramFeature::computeFeatureData(
  ...
  try {
    ...
    for (int r = blkoffset; r < rows - (blksize + blkoffset - 1);
      ...
      for (int c = blkoffset; c < cols - (blksize + blkoffset - 1);
        ...
        if (maskBseg.at<uint8_t>(br, bc) == 1) {
          rvuhist(blkwim, blkorient.at<double>(br, bc), v1sz_x, v1sz_y,
                  this->padFlag, rvures, NanVec);
        }
        bc = bc + 1;
      }
      br = br + 1;
    ...
    addHistogramFeatures(featureDataList, "RVUP_Bin10_", histogramBins10,
                         rvures, 10);
#endif
...
void
rvuhist(Mat block, const double orientation, const int v1sz_x, const int v1sz_y,
        bool padFlag, std::vector<double>& rvures,
        std::vector<uint8_t>& NaNvec) {
  ...
  Mat blockCropped =
      blockRotated(Range((icBlock - (yoff - 1) - 1), (icBlock + yoff)),
  ...
  if (!changeIndex.empty()) {
    ...
    for (int i = changeIndex[0] + 1; i < changeIndex[changeIndex.size() - 1];
         i++) {
    ...
    if (ridvalComplete.empty()) {
      ...
      // If there is only one change index at this point, then there aren't any
      // ridge/valley structures to compare, so leave the ratios vector empty.
      // (This is the same result that Matlab gives, even though it is handled
      // differently, i.e., Matlab handles this edge case within the operations
      // themselves, rather than testing the length of changeIndexComplete
      // directly.
      if (changesize > 1) {
        ...
        for (unsigned int m = 0; m < changeComplete2.size() - 1; m++) {
          r = static_cast<double>(changeComplete2[m]) /
              static_cast<double>(changeComplete2[m + 1]);
          ratios.push_back(r);
