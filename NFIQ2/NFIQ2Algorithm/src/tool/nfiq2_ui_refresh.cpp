...
NFIQ2UI::executeSingle(std::shared_ptr<BE::Image::Image> img,
    ...
    if ((bitDepth != colorDepth) || (bitDepth != 8)) {
        if (flags.force) {
            ...
            if (interactive && !flags.force) {
                ...
                const bool response =
                    NFIQ2UI::yesOrNo(prompt, false, true, true);
    ...
    const uint16_t imageDPI =
        static_cast<uint16_t>(std::round(resolution.xRes));
    ...
    if (singleImage) {
        ...
        logger->printScore(name, fingerPosition, corereturn.qualityScore,
                           warning, quantized, resampled,
    ...
    if (dr != nullptr) {
        ...
        while ((en = readdir(dr)) != nullptr) {
            if (std::string(en->d_name) != "." &&
                ...
                if (flags.recursion && BE::IO::Utility::pathIsDirectory(
                                           dirname + "/" + en->d_name)) {
                    ...
                    if (NFIQ2UI::isRecordStore(NFIQ2UI::removeSlash(dirname) +
                        ...
                        NFIQ2UI::executeRecordStore(
                            NFIQ2UI::removeSlash(dirname) + "/" + en->d_name,
                            flags, model, logger);
                    } else {
                    ...
                    // in the directory
                    if (!BE::IO::Utility::pathIsDirectory(dirname +
                                                          en->d_name)) {
                        logger->debugMsg(
                        ...
                        for (const auto& image : images) {
                            NFIQ2UI::executeSingle(image, flags, model, logger,
                                                   false, true);
                        }
                    }
                }
...
NFIQ2UI::batchConsume(NFIQ2UI::SafeSplitPathsQueue& splitQueue,
    ...
    while (!splitQueue.isEmpty()) {
        ...
        for (const auto& j : item) {
            ...
            for (const auto& image : images) {
                NFIQ2UI::executeSingle(image, flags, model, threadedlogger,
                                       false, false);
        ...
        for (unsigned int i{0}; i < upperThreadBound; ++i) {
            try {
                threads.emplace_back(
                    std::bind(&batchConsume, std::ref(splitQueue),
...
NFIQ2UI::executeRecordStore(const std::string& filename, const Flags& flags,
    ...
    try {
        ...
        std::string error{"'Error: Could not open RecordStore'"};
        logger->printError(filename, 0, 255, error.append(e.what()), false,
                           false);
        return;
    }
    ...
    if (flags.numthreads == 1) {
        ...
        for (const auto& rec : (*rs)) {
            ...
            for (const auto& image : images) {
                logger->debugMsg("Iterating through images in record: " +
                                 rec.key + ", calling executeSingle");

                NFIQ2UI::executeSingle(image, flags, model, logger, false,
                                       false);
            }
        }
    } else {
        ...
        for (unsigned int i{0}; i < upperThreadBound; ++i) {
            try {
                threads.emplace_back(std::bind(
                    &recordStoreConsume, filename, std::ref(splitQueue),
                    std::ref(printQueue), flags, std::cref(model)));
            } catch (const std::exception& e) {
                std::cerr << "Error during thread creation: " << e.what()
                          << "\n";
...
NFIQ2UI::procSingle(NFIQ2UI::Arguments arguments,
    ...
    if (arguments.vecSingle.size() == 1 && arguments.vecDirs.size() == 0 &&
        ...
        for (const auto& image : images) {
            NFIQ2UI::executeSingle(image, arguments.flags, model, logger, true,
                                   true);
        }
    } else {
        for (const auto& i : arguments.vecSingle) {
            ...
            for (const auto& image : images) {
                NFIQ2UI::executeSingle(image, arguments.flags, model, logger,
                                       false, true);
            }
        }
    }
...
main(int argc, char** argv) {
    ...
    try {
        logger =
            std::make_shared<NFIQ2UI::Log>(arguments.flags, arguments.output);
