...
class FingerJetFXFeature : BaseFeature {
 public:
  ...
  struct Object {
    COMType comType;  ///< type of requested centre of mass that shall be used
                      ///< for computation
  ...
  struct MinutiaeInObjectInfo {
    ...
    unsigned int noOfMinutiae;  ///< number of minutiae inside the defined
                                ///< object (e.g. rectangular, circle, ...)
  };

  struct FJFXROIResults {
    unsigned int chosenBlockSize;     ///< the input block size in pixels
    unsigned int noOfCompleteBlocks;  ///< the overall number of complete blocks
                                      ///< (with full block size) in the image
    unsigned int noOfAllBlocks;  ///< the overall number of blocks in the image
    Point
        centreOfMassMinutiae;  ///< Centre of Mass based on minutiae coordinates
    Point centreOfMassGrayValues;  ///< Centre of Mass based on pixel grayvalues
    std::vector<MinutiaeBlockInfo>
        vecROIBlocks;  ///< the detected ROI (that contain at least one minutia)
                       ///< blocks with position, size and number of minutiae
    std::vector<MinutiaeInObjectInfo>
        vecNoOfMinutiaeInRectangular;  ///< vector of the number of minutiae
                                       ///< that are inside the defined
                                       ///< rectangular
    std::vector<MinutiaeInObjectInfo>
        vecNoOfMinutiaeInCircle;  ///< vector of the number of minutiae that are
                                  ///< inside the defined circle
  };
