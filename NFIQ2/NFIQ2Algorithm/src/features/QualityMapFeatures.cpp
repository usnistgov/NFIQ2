...
QualityMapFeatures::computeFeatureData(
  ...
  try {
    ...
    Mat orientationMapImgFilter = computeOrientationMap(
        img, true, coherenceSumFilter, coherenceRelFilter, 16, imgProcResults);
...
cv::Mat
QualityMapFeatures::computeOrientationMap(
    cv::Mat& img, bool bFilterByROI, double& coherenceSum, double& coherenceRel,
    ...
    for (int j = 0; j < img.cols; j += bs) {
      ...
      if (bFilterByROI) {
        ...
        if (!bBlockFound) {
          ...
          continue;  // do not compute angle for a non-ROI block (as no ridge
                     // lines will be there)
        }
      }
...
bool
QualityMapFeatures::getAngleOfBlock(const cv::Mat& block, double& angle,
                                    double& coherence) {
  ...
  for (int k = 0; k < block.rows; k++) {
    for (int l = 0; l < block.cols; l++) {
      ...
      // gx^2 - gy^2
      double gx = ((grad_x.at<double>(k, l) * grad_x.at<double>(k, l)) -
                   (grad_y.at<double>(k, l) * grad_y.at<double>(k, l)));
      if (std::isnan(gx)) {
        sum_x += 0;
      } else {
        sum_x += gx;
...
void
QualityMapFeatures::computeNumericalGradients(const cv::Mat& mat,
                                              cv::Mat& grad_x,
                                              cv::Mat& grad_y) {
  // get x-gradient
