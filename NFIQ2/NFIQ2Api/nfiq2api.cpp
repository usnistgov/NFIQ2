...
InitNfiq2() {
  try {
    if (g_nfiq2.get() == nullptr) {
      ...
      g_nfiq2 =
          std::unique_ptr<NFIQ::NFIQ2Algorithm>(new NFIQ::NFIQ2Algorithm());
#else
      g_nfiq2 = std::unique_ptr<NFIQ::NFIQ2Algorithm>(new NFIQ::NFIQ2Algorithm(
          GetYamlFilePath(), "ccd75820b48c19f1645ef5e9c481c592"));
#endif
...
ComputeNfiq2Score(int fpos, const unsigned char* pixels, int size, int width,
  ...
  try {
    if (g_nfiq2.get() != nullptr) {
      NFIQ::FingerprintImageData rawImage(pixels, size, width, height, fpos,
                                          ppi);
      ...
      int qualityScore = (int)g_nfiq2->computeQualityScore(
          rawImage, true, actionableQuality, false, featureVector, false,
          featureTimings);
      return qualityScore;
    }
  } catch (const NFIQ::NFIQException& exc) {
    std::cerr << "NFIQ2 ERROR => Return code [" << exc.getReturnCode()
