...
class ImgProcROIFeature : BaseFeature {
   public:
    struct ImgProcROIResults {
        ...
        unsigned int noOfCompleteBlocks;     ///< the overall number of complete
                                             ///< blocks (with full block size)
                                             ///< in the image
        unsigned int noOfAllBlocks;          ///< the overall number of blocks
                                             ///< in the image
        std::vector<cv::Rect> vecROIBlocks;  ///< the detected ROI blocks with
                                             ///< position and size
        unsigned int noOfROIPixels;    ///< the number of ROI pixels detected
